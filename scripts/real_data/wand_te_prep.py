#!/usr/bin/env python
"""Build + cache the low-order connectome-harmonic coefficient time series for the
transfer-entropy directed-coupling analysis (run under IDTxl in .venv-oracle).

Geometric harmonics on the Desikan-68 centroid graph -> project the broadband source
MEG -> analytic envelope -> downsample.  Saves harmonic_coeffs.npy (T, N_HARM) for
`wand_transfer_entropy.py`.

    WAND_OUT=/data/datasets/wand_src PYTHONPATH=src \\
      .venv-models/bin/python scripts/real_data/wand_te_prep.py
"""
...
FS = 250.0
TE_FS = 25.0            # downsample for TE tractability (still slow enough for the cycle)
N_HARM = 15
...
def main():
    ...
    Phi_lo = np.asarray(Phi)[:, 1:N_HARM + 1]                       # low-order, drop DC
    ...
    env = (env - env.mean(0)) / (env.std(0) + 1e-9)                 # z-score per harmonic
    np.save(os.path.join(OUT, "harmonic_coeffs.npy"), env.astype(np.float64))
    print(f"harmonic coeffs {env.shape} @ {TE_FS:.0f} Hz -> {OUT}/harmonic_coeffs.npy")
